...
df.sparams <- data.frame("Terms"=c("Sample size", "Nodes",  "Possible edges","Latent processes","Density of the mean network", "$\\sigma_Y$"),
                         "Values"=c(n,p,po,q,round_0(sum(omega.imat!=0)/po*100,2), sparams$eps))
df.sparams  %>%
...
Findings (last updated 31/03/2022):\

+ Approach *density-based* and *bin* showed the worst prediction error due to the similarity of the clustering coefficient across individuals when evaluated for the top X% of edges in which edge weights are set to 1.
+ Approach *FDA* and *trim* showed comparable results to *bRMSE*



```{r, echo=F, waring=F, message=F}
tbl_results <- data.frame(rbind(list_results$tbl_bRMSE, list_results$tbl_AVG, list_results$tbl_FDA))
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=6, fig.height=4, fig.cap="RMSE trajectory of the univariable models including the dependent variable and a graph-theoretical features evaluated at incremental threshold steps for each iteration"}
