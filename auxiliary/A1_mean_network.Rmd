...
```{r}
# Preprocess
# BA.graph <- sample_pa(n=scen$p, power=1, m=50, directed = F)                         # increase m to increase density
...
X1.params = unlist(scen$X1.params, use.names = F)
X2.params = unlist(scen$X2.params, use.names = F)
if(is.list(dg.thresh)){dg.thresh = unlist(scen$dg.thresh)}

# -- Setup default network
dnw.params <- genDefaultNetwork(scen$p, q, BA.graph, beta.params, alpha0.params, alpha12.params, X1.params, X2.params)
```
...
df.sparams <- data.frame("Terms"=c("Sample size", "Nodes",  "Possible edges","Latent processes","Density of the default network"),
                         "Values"=c(as.character(scen$n),scen$p,po,q,d))
df.sparams  %>%
...
alpha12 = dnw.params$alpha$alpha12
etai = alpha0 + c(xi%*%alpha12)
ox=etai
ox[ox!=0] = transform_to_beta(eta=etai[etai!=0], beta.pars = beta.params, eta.pars = dnw.params$eta.params)
obeta0 = rep(1,scen$p)   
...
data.mnet <- wrapperThresholding(mnet_vec, msize=p)
data.mnet <- data.frame(do.call(rbind, data.mnet)) %>%
...
```{r, echo=F, warning=F, message=F}
data.graph <- genIndivNetwork(n=scen$n, p=scen$p, q=scen$q, alpha=dnw.params$alpha,
                              X1.params=unlist(scen$X1.params),X2.params=unlist(scen$X2.params), 
                              mu=dnw.params$mu, beta.params=unlist(scen$beta.params), 
...
```{r, echo=F, warning=F, message=F}
# CC for threshold sequence
list.gvars <- wrapperThresholding(df=data.graph$GE, msize=scen$p)
