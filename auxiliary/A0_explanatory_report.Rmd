...
scen <- scenarios[226,]
po = (scen$p-1)*scen$p/2                                                                  

```

```{r}
# Preprocess
BA.graph <- sample_pa(n=scen$p, power=2, m=20, directed = F)                         # increase m to increase density

beta.params = unlist(scen$beta.params, use.names = F)
alpha0.params = unlist(scen$alpha0.params, use.names = F)
alpha12.params = unlist(scen$alpha12.params, use.names = F)
X.params = unlist(scen$X.params, use.names = F)
da.thresh = unlist(scen$da.thresh, use.names = F)
if(is.list(dg.thresh)){dg.thresh = unlist(scen$dg.thresh)}

# -- Setup default network
dnw.params <- genDefaultNetwork(scen$p, scen$q, BA.graph, beta.params, alpha0.params, alpha12.params, X.params)
```

```{r}
tbl_params <- data.frame("Parameter"=colnames(scen)[c(1:5,7,11:14)],
                         "Values" =as.character(scen[1,c(1:5,7,11:14)]))

tbl_params %>%
  kbl(caption="Parameter of the scenario", escape = F, row.names = F) %>%
  row_spec(0, bold = T) %>%
...
X_1, X_2 \sim N(`r X.params[1]`,`r X.params[2]^2`)
\end{equation}
...
data.graph <- generate_data(n = scen$n, 
                           p = scen$p, 
                           q = scen$q,
...
data.FDA.ps <- data.gvars %>%
  ...
  group_by(SparsMethod, ThreshMethod, Variable) %>%
  nest() %>%
  mutate(res=lapply(data, function(df) evalPFR(data.fda=df, k=k, tseq=da.thresh, bs.type="ps", nodes=30))) %>%
  unnest(res) %>%
...
```{r, fig.align='center', fig.width=8, fig.height=5, fig.cap="Coefficient function of the FDA approach with cubic splines and 10 nodes (default)"}
data.FDA.cr <- data.gvars %>%
  pivot_wider(values_from = Value, names_from = Thresh) %>%
  group_by(SparsMethod, ThreshMethod, Variable) %>%
  nest() %>%
  mutate(res=lapply(data, function(df) evalPFR(data.fda=df, k=k, tseq=da.thresh, bs="cr", nodes=10))) %>%
  unnest(res) %>%
  mutate("AnaMethod"="FDA",
         "Spline"="cr",
         Thresh=as.character(Thresh))

data.FDA.coeff <- data.FDA.cr %>%
  select(Variable, AnaMethod, ThreshMethod, SparsMethod, Coef) %>%
  unnest(Coef) %>%
  reduce(data.frame) %>%
  `colnames<-`(c("Variable", "AnaMethod", "ThreshMethod", "SparsMethod", "fda.thresh", "fda.est", "fda.se"))
...
```{r}
data.frame(rbind(data.FDA.cr, data.FDA.ps)) %>%
  dplyr::select(SparsMethod, ThreshMethod, Spline, Variable, RMSE, R2, CS) %>%
    mutate_at(5:7, as.numeric) %>%
    mutate_at(5:7, round_0, 3) %>%
  arrange(SparsMethod, ThreshMethod) %>%
...
data.results <- data.frame(rbind(
    ...
    data.FDA.ps[,c("AnaMethod","SparsMethod", "ThreshMethod", "Thresh","Variable","RMSE", "R2", "CS")]))
