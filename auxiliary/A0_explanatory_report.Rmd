...
true.flat = rep(b1*2,length(thr.steps))
true.halfsine = ifelse(thr.steps >0.5, 0, sin(thr.steps*pi*2)*b1*2)
...
if(dg.method %in% c("single","random")){
  ...
  mat.gvars <- data.gvars %>%
    ...
    dplyr::select(!ID)
  b1 <- switch(dg.thresh[[1]], "flat"=b1/5, "half-sine"=b1/5, "sine"=b1)
