...
levelplot(mat, xlab="Variable", ylab="Variable", col.regions=rev(gray(0:100/100)))
```
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.caption="Individual-specific variability of graph-theoretical features for XXX individuals"}
...
data.OPT <- data.gvars  %>% 
  ...
  nest() %>%
  mutate(res=lapply(data, function(x) rbind(evalLM_dCV(df=x, k=k, adjust=F), 
                                            evalLM_dCV(df=x, k=k, adjust=T)))) %>%
