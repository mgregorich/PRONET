...
dnw.params <- genDefaultNetwork(n, p, q, beta.params, alpha0.params, alpha12.params, Z1.params, Z2.params)
```
...
data.ews.noisy <- data.graph %>%
  dplyr::select(Z.Z2,paste0("GE.noisy.",1:po)) %>%
  data.frame() %>%
  mutate(ID=1:nrow(data.graph),
         Z.Z2 = as.factor(Z.Z2)) %>%
  pivot_longer(cols=!c(ID,Z.Z2), names_to = "Weights", values_to = "Values") %>%
  filter(Values != 0) %>%
  mutate("Noise"=scn$epslevel.g)
...
mat <- VecToSymMatrix(diag.entries = 1, side.entries = vec_avg, mat.size = p)
...
indiv.graph <- data.graph %>%
  dplyr::select(paste0("GE.",1:po))%>%
  slice_sample(n=sample(1:scn$n,1))

indiv.mat <- VecToSymMatrix(diag.entries = 0, side.entries = as.vector(indiv.graph), mat.size = scn$p)
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=6, fig.height=4, fig.cap="Variability of edge weights per edge across individuals"}
data.graph %>%
    dplyr::select(paste0("GE.",1:200)) %>%
    melt() %>%
    filter(value!=0) %>%
...
if(scn$epslevel.g == "none"){ data.gvars.true <- data.gvars
}else{data.gvars.true <- wrapperThresholding(df=data.network.true, msize=p, step.size=step.size)}


# Add outcome Y
data.gvars <- merge(data.graph[,c("ID","Y", "X", "fold")], data.gvars, by="ID") %>%
  mutate(Value=ifelse(is.nan(Value), NA, Value),
         Noise = scn$epslevel.g) %>%
...
data.gvars.true <- merge(data.graph[,c("ID","Y", "X", "fold")], data.gvars.true, by="ID") %>%
  ...
  mutate_at(vars(Thresh, Y, Value), as.numeric) %>%
  filter(Variable %in% "cc.uw") 
data.gvars.all <- data.frame(rbind(data.gvars, data.gvars.true)) %>%
  mutate(ID_NOISE=paste0(ID, "_", Noise))
```
...
data.gvars.all %>% 
 # filter(ID %in% sample(unique(ID),1)) %>%
  ggplot(aes(x=Thresh, y=Value, group=ID_NOISE, col=Noise, fill=Noise)) +
  geom_line() +
...
true.opt <- data.frame("ID"=scn$n, "thresh"=thr.steps, "value"=ifelse(thr.steps==0.25, scn$b1, NA), "DGM"="single")

true.hs <- data.frame("ID"=scn$n, "thresh"=thr.steps, "value"=true.halfsine, "DGM"="half-sine")
...
data.results %>%
  ...
  mutate_at(7:10, as.numeric) %>%
    mutate_at(7:10, round_0, 3) %>%
  arrange(RMSPE) %>%
