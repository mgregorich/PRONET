...
data.graph %>%
  ...
  filter(Values != 0) %>%
  ggplot(aes(x=Values, group=ID, col=Z.Z2)) +
  geom_density() +
...
data.OPT <- data.gvars  %>% 
  ...
  nest() %>%
  mutate(res=lapply(data, function(df) evalLM_dCV(df=df,k=k))) %>%
...
data.FLEX <- data.gvars %>%
  ...
  group_by(SparsMethod, ThreshMethod, Variable) %>%
  nest() %>%
  mutate(res=lapply(data, function(df) evalPFR(df=df, k=k, bs.type="ps", nodes=20))) %>%
  unnest(res) %>%
...
data.FLEX.cs <- data.gvars %>%
  ...
  group_by(SparsMethod, ThreshMethod, Variable) %>%
  nest() %>%
  mutate(res=lapply(data, function(df) evalPFR(df=df, k=k, bs.type="cs", nodes=5))) %>%
  unnest(res) %>%
  mutate("AnaMethod"="FLEX",
         "Spline"="cs",
         Thresh=as.character(Thresh))
