...
data.FDA.cs <- data.gvars %>%
  arrange(Thresh) %>%
  mutate(Thresh=paste0("T_",Thresh)) %>%
  pivot_wider(values_from = Value, names_from = Thresh) %>%
  group_by(SparsMethod, ThreshMethod, Variable) %>%
  nest() %>%
  mutate(res=lapply(data, function(df) evalPFR(data.fda=df, k=k, bs.type="bs", nodes=20))) %>%
  unnest(res) %>%
  mutate("AnaMethod"="FDA",
         "Spline"="cs",
         Thresh=as.character(Thresh))
...
### Estimated functional form of beta(t)
...
data.FDA.coeff %>%
  ...
  scale_x_continuous("Threshold") +
  theme_bw() +
  facet_grid(~SparsMethod, scales="free_y")
```
...
```{r, fig.align='center', fig.width=8, fig.height=5, fig.cap="Coefficient function of the FDA approach with cubic splines and 5 nodes"}
data.FDA.coeff <- data.FDA.cs %>%
  select(Variable, AnaMethod, ThreshMethod, SparsMethod, Coef) %>%
  unnest(Coef) %>%
  reduce(data.frame) %>%
  `colnames<-`(c("Variable", "AnaMethod", "ThreshMethod", "SparsMethod",
...
data.results <- data.frame(rbind(
    ...
    data.bRMSE[,c("AnaMethod","SparsMethod", "ThreshMethod", "Thresh","Variable","Spline","RMSE", "R2", "CS")],
    data.AVG[,c("AnaMethod","SparsMethod", "ThreshMethod", "Thresh","Variable","Spline","RMSE", "R2", "CS")],
    data.FDA[,c("AnaMethod","SparsMethod", "ThreshMethod", "Thresh","Variable","Spline","RMSE", "R2", "CS")],
    data.FDA.cs[,c("AnaMethod","SparsMethod", "ThreshMethod", "Thresh","Variable","Spline","RMSE", "R2", "CS")]))
