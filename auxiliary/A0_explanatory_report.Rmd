...
tbl_params <- data.frame("Parameter"=colnames(scn)[c(1:5,7,11:14)],
                         "Values" =unlist(sapply(scn[1,c(1:5,7,11:14)], as.character)))
...
data.FLEX <- data.gvars %>%
  ...
  group_by(SparsMethod, ThreshMethod, Variable) %>%
  nest() %>%
  mutate(res=lapply(data, function(df) evalPFR(df=df, k=k, bs.type="ps", fx=T))) %>%
  unnest(res) %>%
  mutate("AnaMethod"="FLEX",
         "Spline"="csT",
         Thresh=as.character(Thresh))

data.FLEX.F <- data.gvars %>%
  arrange(Thresh)%>%
  mutate(Thresh=paste0("T_",Thresh)) %>%
  pivot_wider(values_from = Value, names_from = Thresh) %>%
  group_by(SparsMethod, ThreshMethod, Variable) %>%
  nest() %>%
  mutate(res=lapply(data, function(df) evalPFR(df=df, k=k, bs.type="ps", fx=F))) %>%
  unnest(res) %>%
  mutate("AnaMethod"="FLEX",
         "Spline"="csF",
         Thresh=as.character(Thresh))
...
data.results <- data.frame(rbind(
    ...
    data.FLEX.F[,c("AnaMethod","SparsMethod", "ThreshMethod", "Thresh","Variable","Spline","RMSE", "R2", "CS")]))
