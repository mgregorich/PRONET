...
compute_scnR2 <- function(scn){
  # scn = data.frame(scenarios.single[5,])
...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(setting %in% "uni" & n==N & data_gen_mech == "single" & 
                                           data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
...
```{r random_u}
# --- Scenario: random threshold U(0.1,0.4)
scenarios.random <-  scenarios %>% filter(setting %in% "uni" & n==N & data_gen_mech == "random" & data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
...
scenarios.func1 <-  scenarios %>% filter(setting %in% "uni" & n==N & data_gen_mech == "flat" & data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
...
```{r halfsine_u}
# --- Scenario: functional form
scenarios.func2 <- scenarios %>% filter(setting %in% "uni" & n==N & data_gen_mech == "half-sine" & data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
...
```{r sine_u}
# --- Scenario: functional form
scenarios.func3 <- scenarios %>% filter(setting %in% "uni" & n==N & data_gen_mech == "half-sine" & data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(setting %in% "latent" & n==N & data_gen_mech == "single" & 
                                           data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
...
# --- Scenario: random threshold U(0.1,0.4)
scenarios$R2 <- NA
scenarios.random <- scenarios %>% filter(setting %in% "latent" & n==N & data_gen_mech == "random" & 
                                           data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
...
```{r flat_l}
# --- Scenario: functional form
scenarios.func1 <- scenarios %>% filter(setting %in% "latent" & n==N & data_gen_mech == "flat" & 
                                           data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
scenarios.func1$R2 <- future_sapply(1:nrow(scenarios.func1), function(x) compute_scnR2(data.frame(scenarios.func1[x,])), future.seed=TRUE)
scenarios.func1$R2 <- round(scenarios.func1$R2,2)
...
```{r halfsine_l}
# --- Scenario: functional form
scenarios.func2 <- scenarios %>% filter(setting %in% "latent" & n==N & data_gen_mech == "half-sine" & 
                                           data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
scenarios.func2$R2 <- future_sapply(1:nrow(scenarios.func2), function(x) compute_scnR2(data.frame(scenarios.func2[x,])), future.seed=TRUE)
scenarios.func2$R2 <- round(scenarios.func2$R2,2)
...
```{r sine_l}
# --- Scenario: functional form
scenarios.func3 <- scenarios %>% filter(setting %in% "latent" & n==N & data_gen_mech == "sine" & 
                                           data_gen_thresh == "weight-based" & data_gen_feature == "cpl")
plan(multisession, workers=detectCores()*.75)
scenarios.func3$R2 <- future_sapply(1:nrow(scenarios.func3), function(x) compute_scnR2(data.frame(scenarios.func3[x,])), future.seed=TRUE)
scenarios.func3$R2 <- round(scenarios.func3$R2,2)
...
plan(sequential)
xtabs(R2~eps_y+eps_g, data = scenarios.diag)
```
