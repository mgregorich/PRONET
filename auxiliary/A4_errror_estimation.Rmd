...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(n==N & p==P & names(dg.thresh) == "single")
...
```{r random}
# --- Scenario: random threshold U(0.1,0.4)
scenarios.random <- scenarios %>% filter(n==N & p==P & names(dg.thresh) == "random")
...
scenarios.func1 <- scenarios %>% filter(n==N & p==P & dg.thresh == "flat")
...
```{r halfsine}
# --- Scenario: functional form
scenarios.func2 <- scenarios %>% filter(n==N & p==P & dg.thresh == "half-sine")
...
```{r sine}
# --- Scenario: functional form
scenarios.func3 <- scenarios %>% filter(n==N & p==P & dg.thresh == "sine")
