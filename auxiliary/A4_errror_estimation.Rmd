...
compute_scnR2 <- function(scn){
  ...
  if(dg.method %in% c("single","random")){
    ...
    b1 <- switch(dg.method, "flat"=b1/5,"half-sine"=b1/5, "sine"=b1)

    data.oracle <- data.gvars %>%
      filter(SparsMethod == true.params$SparsMethod & ThreshMethod == true.params$ThreshMethod &
               Variable == true.params$Variable & Thresh == 0) %>%
      mutate(Value = prod.betaX.true*b1)
  }
...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(n==N & p==P & network.model=="block" & names(dg.thresh) == "single" & setting == "uni")
...
```{r random_u}
# --- Scenario: random threshold U(0.1,0.4)
scenarios.random <- scenarios %>% filter(n==N & p==P & network.model=="block"  & names(dg.thresh) == "random" & setting == "uni")
...
scenarios.func1 <- scenarios %>% filter(n==N & p==P & network.model=="block" & dg.thresh == "flat" & setting == "uni")
plan(multisession, workers=detectCores()*.75)
...
```{r halfsine_u}
# --- Scenario: functional form
scenarios.func2 <- scenarios %>% filter(n==N & p==P & network.model=="block" & dg.thresh == "half-sine" & setting == "uni")
plan(multisession, workers=detectCores()*.75)
...
```{r sine_u}
# --- Scenario: functional form
scenarios.func3 <- scenarios %>% filter(n==N & p==P & network.model=="block" & dg.thresh == "sine" & setting == "uni")
plan(multisession, workers=detectCores()*.75)
...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(n==N & p==P & network.model=="block" & names(dg.thresh) == "single"  & setting == "latent")
...
# --- Scenario: random threshold U(0.1,0.4)
scenarios$R2 <- NA
scenarios.random <- scenarios %>% filter(n==N & p==P & network.model=="block" & names(dg.thresh) == "random"  & setting == "latent")
...
```{r flat_l}
# --- Scenario: functional form
scenarios.func1 <- scenarios %>% filter(n==N & p==P & network.model=="block"  & dg.thresh == "flat" & setting == "latent")
plan(multisession, workers=detectCores()*.75)
scenarios.func1$R2 <- future_sapply(1:nrow(scenarios.func1), function(x) compute_scnR2(data.frame(scenarios.func1[x,])), future.seed=TRUE)
scenarios.func1$R2 <- round(scenarios.func1$R2,2)
plan(sequential)
...
```{r halfsine_l}
# --- Scenario: functional form
scenarios.func2 <- scenarios %>% filter(n==N & p==P & network.model=="block" & dg.thresh == "half-sine" & setting == "latent")
plan(multisession, workers=detectCores()*.75)
scenarios.func2$R2 <- future_sapply(1:nrow(scenarios.func2), function(x) compute_scnR2(data.frame(scenarios.func2[x,])), future.seed=TRUE)
scenarios.func2$R2 <- round(scenarios.func2$R2,2)
plan(sequential)
...
```{r sine_l}
# --- Scenario: functional form
scenarios.func3 <- scenarios %>% filter(n==N & p==P & network.model=="block" & dg.thresh == "sine" & setting == "latent")
plan(multisession, workers=detectCores()*.75)
scenarios.func3$R2 <- future_sapply(1:nrow(scenarios.func3), function(x) compute_scnR2(data.frame(scenarios.func3[x,])), future.seed=TRUE)
scenarios.func3$R2 <- round(scenarios.func3$R2,2)
plan(sequential)
...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(n==N & p==P & network.model=="block" & names(dg.thresh) == "single"  & setting == "multi")
...
# --- Scenario: random threshold U(0.1,0.4)
scenarios$R2 <- NA
scenarios.random <- scenarios %>% filter(n==N & p==P & network.model=="block" & names(dg.thresh) == "random"  & setting == "multi")
...
```{r flat_m}
# --- Scenario: functional form
scenarios.func1 <- scenarios %>% filter(n==N &  p==P & network.model=="block" &dg.thresh == "flat" & setting == "multi")
...
```{r halfsine_m}
# --- Scenario: functional form
scenarios.func2 <- scenarios %>% filter(n==N & p==P & network.model=="block" & setting == "multi" & dg.thresh == "half-sine")
...
```{r sine_m}
# --- Scenario: functional form
scenarios.func3 <- scenarios %>% filter(n==N & p==P & network.model=="block" & setting == "multi" & dg.thresh == "sine" )
