...
compute_scnR2 <- function(scn){
  ...
  for(i in 1:rps){
    ...
    data.gen <- generate_data(setting=scn$setting,
                        ...
                        p = scn$p, 
                        q = scn$q,
                        alpha0.params = unlist(scn$alpha0.params),
                        alpha12.params = unlist(scn$alpha12.params),
    ...
    if(scn$outcome %in% "diagnostic"){
      R2[i] <- cor(data.graph$Y, data.graph$Z.Z1)
    }else{
      # Estimation of the graph-theoretical features
      k=5
      data.network <- data.graph[,paste0("GE.noisy.",1:po)]
      data.graph$ID <- 1:n
      data.graph$fold <- sample(rep(1:5, ceil(n/k)))[1:n]
...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(n==N & p==P &  names(dg.thresh) == "single" & setting == "uni" & outcome == "prognostic")
...
```{r random_u}
# --- Scenario: random threshold U(0.1,0.4)
scenarios.random <- scenarios %>% filter(n==N & p==P & names(dg.thresh) == "random" & setting == "uni" & outcome=="prognostic")
...
scenarios.func1 <- scenarios %>% filter(n==N & p==P  & dg.thresh == "flat" & setting == "uni" & outcome=="prognostic")
plan(multisession, workers=detectCores()*.75)
...
```{r halfsine_u}
# --- Scenario: functional form
scenarios.func2 <- scenarios %>% filter(n==N & p==P & dg.thresh == "half-sine" & setting == "uni" & outcome=="prognostic")
plan(multisession, workers=detectCores()*.75)
...
```{r sine_u}
# --- Scenario: functional form
scenarios.func3 <- scenarios %>% filter(n==N & p==P & dg.thresh == "sine" & setting == "uni" & outcome=="prognostic")
plan(multisession, workers=detectCores()*.75)
...
# --- Scenario: single threshold = 0.25
scenarios$R2 <- NA
scenarios.single <- scenarios %>% filter(n==N & p==P & names(dg.thresh) == "single"  & setting == "latent" & outcome=="prognostic")
