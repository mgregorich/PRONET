...
setting = c("uni", "multi")                                                     # setting: univariable or multivariable modelling
...
scenarios <- scenarios %>%
  ...
  mutate(eps.g = case_when(names(dg.thresh) %in% c("single", "flat", "sine", "half-sine") & epslevel.g %in% "medium"~ 0.3,
         ...
         eps.y = case_when(names(dg.thresh) %in% c("random") & epslevel.y %in% "medium" ~ 2.5,
                           ...
                           names(dg.thresh) %in% c("single", "half-sine", "flat", "sine") & epslevel.y %in% "medium" ~ 2,
                           names(dg.thresh) %in% c("single", "half-sine","flat", "sine") & epslevel.y %in% "high"~ 4,
                           TRUE ~ 0),
