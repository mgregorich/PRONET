...
scenarios.prognostic <- scenarios %>%
  ...
  mutate(eps.y = 0,
         eps.g = 0) %>%
  mutate(eps.y = case_when(names(dg.thresh) %in% c("single", "flat", "half-sine", "sine") & epslevel.y %in% "medium" ~ 2.5,
                           names(dg.thresh) %in% c("single", "flat", "half-sine", "sine") & epslevel.y %in% "high"~ 5,
                           names(dg.thresh) %in% c("random") & epslevel.y %in% "medium" ~ 3,
                           names(dg.thresh) %in% c("random") & epslevel.y %in% "high"~ 6,
                           TRUE ~ 0),
         eps.g = case_when(names(dg.thresh) %in% c("single") & epslevel.g %in% "medium" ~ 0.4,
                           names(dg.thresh) %in% c("single") & epslevel.g %in% "high" ~ 0.8,
                           names(dg.thresh) %in% c("random", "flat", "half-sine", "sine") & epslevel.g %in% "medium" ~ 0.5,
                           names(dg.thresh) %in% c("random", "flat", "half-sine", "sine") & epslevel.g %in% "high" ~ 1,
                           TRUE ~ 0),         
