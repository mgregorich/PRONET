...
# Paths
out.path <- "output"
sim.file <- "sim_2022-05-10/"              #paste0("sim_", Sys.Date(),"/")
...
tbl_scens %>%
  filter(ThreshMethod %in% "trim" & eps.g == nth(eps.g,3)) %>%
  mutate_at(vars(beta.params, SparsMethod, AnaMethod), as.factor) %>%
  mutate(beta.params = paste0("beta pars = ", beta.params)) %>%
  ggplot(aes(x=eps.y, y=RMSE.est, group=AnaMethod, col=AnaMethod)) +
  geom_ribbon(aes(x=eps.y, ymax=RMSE.up, ymin=RMSE.lo, fill=AnaMethod), linetype=2,alpha=.05) +
...
```{r, echo=F, warning=F, message=F,  fig.align='center', fig.width=10, fig.height=6, fig.cap="Nested line plot for RMSE for ThreshMethod = trim and fixed large eps.g"}
tbl_scens %>%
  filter(ThreshMethod %in% "trim" & eps.g == nth(eps.g,length(eps.g))) %>%
  mutate_at(vars(beta.params, SparsMethod, AnaMethod), as.factor) %>%
  mutate(beta.params = paste0("beta pars = ", beta.params)) %>%
  ggplot(aes(x=eps.y, y=RMSE.est, group=AnaMethod, col=AnaMethod)) +
  geom_ribbon(aes(x=eps.y, ymax=RMSE.up, ymin=RMSE.lo, fill=AnaMethod), linetype=2,alpha=.05) +
