...
tbl_scens <- tbl_res$tbl_scens %>%
  ...
  mutate_at(vars(dg.thresh, epslevel.g, epslevel.y, n, p), to_factor) %>%
...
## Analyses with independent variable

```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE, eval=eval_multi}
cond <- quo((setting=="multi" & outcome=="prognostic"  & epslevel.g=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE, eval=eval_multi}
cond <- quo((setting=="multi" & outcome=="prognostic" & epslevel.y=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE, eval=eval_multi}
cond <- quo((setting=="multi" & outcome=="prognostic" &  epslevel.y=="none" & Adjust==F))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
cond <- quo((setting=="latent" & outcome=="diagnostic"  & Adjust==T & DGM =="single"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_scens %>%
  filter(network.model=="block"  & SparsMethod == "weight-based" & outcome =="diagnostic" & AnaMethod != "Oracle") %>%
...
```{r, eval=eval_diagnostic}
tbl_scens %>%
  filter(network.model=="block"  & SparsMethod == "density-based" & outcome =="diagnostic" & AnaMethod != "Oracle") %>%
  dplyr::select(setting, n, Adjust, AnaMethod,  epslevel.y, epslevel.g, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(setting, n, Adjust, AnaMethod,  epslevel.y, epslevel.g, RMSPE) %>%
...
cond <- quo((SparsMethod == "weight-based" & outcome=="diagnostic"  & DGM=="single") & 
                     ((Adjust==F & setting %in% c("uni"))| (Adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo1 <- tbl_funs %>%
  ...
  mutate(true.R2 = as.numeric(true.R2)) 
...
tbl_funcoeff %>%
  filter(outcome=="diagnostic"  & setting =="latent" & Adjust==F & epslevel.g =="none" & n==150) %>%
  rename("Noise.Y"=epslevel.y) %>%
