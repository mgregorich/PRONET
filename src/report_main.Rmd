...
# Paths
out.path <- "output"
sim.date <- "2023-02-13" # Sys.Date()
sim.file <- paste0("sim_", sim.date,"/")
sim.path <- here::here(out.path, sim.file)
ggcols <- c("black", "deepskyblue3", "royalblue3", "mediumorchid3","grey60")
nfig=1
...
```{r, fig.align='center', fig.width=10, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
tbl_res$tbl_iters_res %>%
  filter(setting=="latent" & outcome=="prognostic" & epslevel.y=="medium" & Adjust==T  & n==150 & SparsMethod %in% "density-based" & AnaMethod != "Oracle") %>%
  mutate(AnaMethod = fct_relevel(AnaMethod, c("Oracle", "OPT", "AVG", "FLEX", "Null")),
         epslevel.g = paste0("Noise(G) = ", epslevel.g),
         epslevel.g = fct_relevel(epslevel.g, c("Noise(G) = none", "Noise(G) = medium", "Noise(G) = high")),
         dg.thresh = fct_relevel(dg.thresh, c("single", "random", "flat", "half-sine", "sine"))) %>%
...
```{r, fig.align='center', fig.width=10, fig.height=8, fig.cap="", fig.fullwidth = TRUE, eval=eval_diagnostic}
