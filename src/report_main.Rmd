...
tbl_scens <- tbl_res$tbl_scens %>%
  data.frame() %>%
  filter(beta.params %in% "beta(1.3, 6)") %>%
  dplyr::rename("DGM"=dg.thresh) %>%
  mutate_at(vars(eps.g, eps.y, n, p), as.numeric) %>% 
  mutate_at(vars(DGM, epslevel.g, epslevel.y, n, p), to_factor) %>%
  mutate(DGM = fct_relevel(DGM, c("single", "random", "flat", "half-sine", "sine")))   %>%
  mutate(n = fct_relevel(n, c("75", "150", "300"))) %>%
  mutate(epslevel.g = fct_relevel(epslevel.g, c("none", "medium", "high"))) %>%
  mutate(epslevel.y = fct_relevel(epslevel.y, c("none", "medium", "high"))) %>%
  mutate(AnaMethod = fct_relevel(AnaMethod, c("Oracle", "OPT", "AVG", "FLEX", "Null"))) 
...
tbl_funcoeff <- tbl_res$tbl_funcoeff %>%
  ...
  dplyr::rename("DGM"=dg.thresh)%>%
  mutate_at(vars(n, p), as.numeric) %>% 
  mutate_at(vars(DGM, epslevel.g, epslevel.y, n, p), as.factor) %>%
  mutate(DGM = fct_relevel(DGM, c("single", "random", "flat", "half-sine", "sine"))) %>%
  mutate(n = fct_relevel(n, c("75", "150", "300"))) %>%
  mutate(epslevel.g = fct_relevel(epslevel.g, c("none", "medium", "high"))) %>%
  mutate(epslevel.y = fct_relevel(epslevel.y, c("none", "medium", "high"))) 
...
eval_diagnostic <- "diagnostic" %in% unique(tbl_scens$outcome)
```
...
<!-- ```{r} -->
<!-- tmp_150 <- tbl_res$tbl_iters_res %>% -->
<!--   filter(n==150 & setting =="latent" & epslevel.y %in% "none" & epslevel.g %in% "medium" & Adjust==T & outcome =="prognostic" & SparsMethod %in% "weight-based" & dg.thresh %in% "half-sine" ) -->

<!-- tmp_300 <- tbl_res$tbl_iters_res %>% -->
<!--   filter(n==300 & setting =="latent" & epslevel.y %in% "none" & epslevel.g %in% "medium" & Adjust==T & outcome =="prognostic" & SparsMethod %in% "weight-based" & dg.thresh %in% "half-sine" ) -->
<!-- hist(tmp_150$RMSPE) -->
<!-- hist(tmp_300$RMSPE) -->
...
cond <- quo((setting=="uni" & outcome == "prognostic" & epslevel.g=="none" & Adjust==T))
...
tbl_loo1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(SparsMethod,n, DGM, epslevel.y, AnaMethod, RMSPE.est) %>%
  arrange(AnaMethod) %>%
  pivot_wider(id_cols = c(SparsMethod, n, DGM, epslevel.y), names_from = AnaMethod, values_from = RMSPE.est) 

p = nested_loop_plot(resdf = tbl_loo1,
                     x = "n", steps = "epslevel.y",
                     legend_labels = c("Oracle", "OPT", "AVG", "FLEX", "Null"),
                     grid_rows="SparsMethod", grid_cols = "DGM",
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((setting=="uni" & outcome=="prognostic" & epslevel.y=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(SparsMethod, n, DGM, epslevel.g, AnaMethod, RMSPE.est) %>%
  arrange(AnaMethod) %>%
  pivot_wider(id_cols = c(SparsMethod, n, DGM, epslevel.g), names_from = AnaMethod, values_from = RMSPE.est) 
...
nfig = nfig+1
print(p)
# ggsave(file = paste0(sim.path, "/fig_loo_RMSPE_epsG.tiff"),
#        width=1.75*200, height=1*200, units="mm", dpi=350, compression = "lzw")
```
...
<!--   filter(SparsMethod %in% "weight-based" & outcome %in% "prognostic" & epslevel.y=="none" & Adjust==T & setting =="latent" & epslevel.g=="medium" & dg.thresh=="half-sine" ) %>% -->
<!-- ggplot(aes(x=n, y=RMSPE, col=AnaMethod)) + -->
<!--   geom_point() + -->
<!--   theme_bw() -->
<!-- ``` -->
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((setting=="uni" & outcome=="prognostic" & epslevel.y=="high" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_res$tbl_iters_res %>%
  filter(setting=="uni" & outcome=="prognostic" & epslevel.y=="medium" & Adjust==T  & n==150) %>%
  mutate(AnaMethod = fct_relevel(AnaMethod, c("Oracle", "OPT", "AVG", "FLEX", "Null")),
         epslevel.g = paste0("Noise(G) = ", epslevel.g),
         epslevel.g = fct_relevel(epslevel.g, c("Noise(G) = none", "Noise(G) = medium", "Noise(G) = high"))) %>%
  ggplot(aes(x=AnaMethod, y=relRMSPE, fill=AnaMethod)) + geom_line(aes(group=AnaMethod), alpha=.05) +
         ...
         facet_grid(SparsMethod~epslevel.g) +
         theme(axis.text.x=element_text(angle=40, hjust=1), legend.position="none")  +
         labs(x="", y="Relative RMSPE")
ggsave(file = paste0(sim.path, "/fig_uni_relRMSPE.tiff"),
...
cond <- quo((setting=="latent" & outcome=="prognostic"  & epslevel.g=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE, eval=eval_latent}
cond <- quo((setting=="latent" & outcome=="prognostic" & epslevel.y=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE, eval=eval_latent}
cond <- quo((setting=="latent" & outcome=="prognostic" & epslevel.y=="none" & Adjust==T & n==150))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_noiseG <- tbl_iters %>%
  filter(!!cond) %>%
  dplyr::select(SparsMethod, DGM, epslevel.g, AnaMethod, RMSPE) %>%
  arrange(AnaMethod)

tbl_noiseG %>%
  ggplot(aes(x=epslevel.g, y=RMSPE, group=AnaMethod, col=AnaMethod)) +
  geom_boxplot() +
  scale_color_manual(values=ggcols) +
#  geom_line() +
#  geom_point() +
  facet_grid(DGM~SparsMethod) +
  theme_bw()
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE, eval=eval_latent}
cond <- quo((setting=="latent" & outcome=="prognostic" &  epslevel.y=="none" & Adjust==F))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=10, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
tbl_res$tbl_iters_res %>%
  filter(setting=="latent" & outcome=="prognostic" & epslevel.y=="medium" & Adjust==T  & n==150 & SparsMethod %in% "weight-based") %>%
  mutate(AnaMethod = fct_relevel(AnaMethod, c("Oracle", "OPT", "AVG", "FLEX", "Null")),
         epslevel.g = paste0("Noise(G) = ", epslevel.g),
         epslevel.g = fct_relevel(epslevel.g, c("Noise(G) = none", "Noise(G) = medium", "Noise(G) = high")),
         dg.thresh = fct_relevel(dg.thresh, c("single", "random", "flat", "half-sine", "sine"))) %>%
  ggplot(aes(x=AnaMethod, y=relRMSPE, fill=AnaMethod)) + geom_line(aes(group=AnaMethod), alpha=.05) +
         stat_boxplot(geom = "errorbar", width = 0.25) + geom_boxplot(alpha=0.4, width=0.5) +  
         theme_bw(base_size=16) + scale_fill_manual(values=ggcols) +
         facet_grid(epslevel.g~dg.thresh) +
         theme(axis.text.x=element_text(angle=40, hjust=1), legend.position="none")  +
...
```{r, fig.align='center', fig.width=10, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
tbl_res$tbl_iters_res %>%
  filter(setting=="latent" & outcome=="prognostic" & epslevel.y=="none" & Adjust==T  & n==300 & SparsMethod %in% "density-based" & AnaMethod != "Oracle") %>%
  mutate(AnaMethod = fct_relevel(AnaMethod, c("Oracle", "OPT", "AVG", "FLEX", "Null")),
         epslevel.g = paste0("Noise(G) = ", epslevel.g),
         epslevel.g = fct_relevel(epslevel.g, c("Noise(G) = none", "Noise(G) = medium", "Noise(G) = high")),
         dg.thresh = fct_relevel(dg.thresh, c("single", "random", "flat", "half-sine", "sine"))) %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "density-based"  & epslevel.g == "none" & epslevel.y == "none" & outcome =="prognostic") %>%
  dplyr::select(DGM, setting, Adjust, AnaMethod, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(DGM, setting, Adjust, AnaMethod, RMSPE) %>%
  arrange(AnaMethod) %>%
  pivot_wider(names_from = AnaMethod, values_from = RMSPE) %>%
  relocate(setting, .before=DGM) %>%
  mutate(DGM=fct_relevel(DGM, "single", "random", "flat", "half-sine", "sine")) %>%
  arrange(setting,DGM, Adjust) %>%
  kbl(caption="aRMSPE acrosss setings and data-generating mechanisms with 95% CI: no noise(G) and noise(Y)", escape = F) %>%
  row_spec(0, bold = T) %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "density-based"  & epslevel.g == "none" & epslevel.y == "high"& outcome =="prognostic") %>%
  dplyr::select(DGM, setting, Adjust, AnaMethod, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(DGM, setting, Adjust, AnaMethod, RMSPE) %>%
  ...
  kbl(caption="aRMSPE acrosss setings and data-generating mechanisms with 95% CI: no noise(G) and high noise(Y)", escape = F) %>%
  row_spec(0, bold = T) %>%
  column_spec(1, bold=T) %>%
  column_spec(c(3), border_right = T) %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "density-based"  & epslevel.g == "high" & epslevel.y == "none"& outcome =="prognostic") %>%
  dplyr::select(DGM, setting, Adjust, AnaMethod, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(DGM, setting, Adjust, AnaMethod, RMSPE) %>%
  ...
  mutate(DGM=fct_relevel(DGM, "single", "random", "flat", "half-sine", "sine")) %>%
  arrange(setting, Adjust ,DGM) %>%
  kbl(caption="aRMSPE acrosss setings and data-generating mechanisms with 95% CI: high noise(G) and no noise(Y)", escape = F) %>%
  row_spec(0, bold = T) %>%
  column_spec(1, bold=T) %>%
  column_spec(c(3), border_right = T) %>%
 # row_spec(c(5,10,15,20), extra_css ="border-bottom: 1px solid") %>%
...
```{r}
tbl_scens %>%
  filter(n==150 & SparsMethod == "density-based"  & epslevel.g == "high" & epslevel.y == "high" & outcome =="prognostic") %>%
  dplyr::select(DGM, setting, Adjust, AnaMethod, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(DGM, setting, Adjust, AnaMethod, RMSPE) %>%
...
cond <- quo((SparsMethod == "weight-based" & outcome=="prognostic"  & epslevel.g=="none") & 
...
tbl_loo1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(setting,  n, DGM, epslevel.y, AnaMethod, R2.est) %>%
  arrange(AnaMethod) %>%
  pivot_wider(id_cols = c(setting, n, DGM, epslevel.y), names_from = AnaMethod, values_from = R2.est)
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & outcome=="prognostic"  & epslevel.y=="none") & 
                     ((Adjust==F & setting %in% c("uni"))| (Adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(setting,  n, DGM, epslevel.g, AnaMethod, R2.est) %>%
  arrange(AnaMethod) %>%
  pivot_wider(id_cols = c(setting, n, DGM, epslevel.g), names_from = AnaMethod, values_from = R2.est) 
...
cond <- quo(SparsMethod == "weight-based" & outcome=="prognostic"  & setting =="uni" & Adjust==F & DGM %in% c("single"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  ...
  group_by(n, p, DGM, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, p, epslevel.y, epslevel.g), names_from = DGM, values_from = wmean) %>%
...
p = nested_loop_plot(resdf = tbl_tfreq_loo,
                     ...
                     spu_x_shift = 1.5,
                     legend_name = "DGM",
                     ylim = c(0.1,0.5), y_breaks = seq(0.1,0.5,0.05),
...
ggsave(file = paste0(sim.path, "/fig_OPT_threshold.tiff"),
       width=1.75*100, height=1*100, units="mm", dpi=350, compression = "lzw")
```
<br>
...
tbl_loo5 <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic"  & setting =="uni" & Adjust==F & DGM %in% c("single")) %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, DGM, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, epslevel.y, epslevel.g), names_from = DGM, values_from = wmean) %>%
  rename("Noise(Y)"=epslevel.y)
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic"  & setting =="uni" & Adjust==F & DGM %in% "random") %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, DGM, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, epslevel.y, epslevel.g), names_from = DGM, values_from = wmean) %>%
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic"  & setting =="uni" & Adjust==F) %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, p, DGM, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, p, epslevel.y, epslevel.g), names_from = DGM, values_from = wmean) %>%
...
true.sine = ifelse(thr.steps >0.75, 0, sine_fn(thr.steps))
...
tbl_funcoeff %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic"  & setting =="uni" & Adjust==F & epslevel.y =="none" & n==150) %>%
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.cap="Average functional form \\beta (t)\\ with the estimated flexible parametrization"}
tbl_funcoeff %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic"  & setting =="uni" & DGM %in% c("single", "random", "half-sine") & Adjust==F & epslevel.y =="none" & n==300) %>%
  rename("Noise.G"=epslevel.g) %>%
  ggplot(aes(x=fda.thresh, y=coeff.mean.ustd)) +
  geom_line() +
  scale_x_continuous("Threshold", breaks = seq(0,1,0.2)) +
...
tbl_funs %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic"  & setting =="uni" & Adjust==F & epslevel.g=="none" & epslevel.y =="none" & !(DGM %in% c("single", "random"))) %>%
...
## Results
Oracle does not exist in this setting.

```{r, fig.align='center', fig.width=8, fig.height=6, fig.cap="", fig.fullwidth = TRUE, eval=eval_diagnostic}
cond <- quo((outcome=="diagnostic"  & Adjust==T & !AnaMethod %in% "Oracle"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(SparsMethod,n, epslevel.y, epslevel.g, AnaMethod, RMSPE.est) %>%
  arrange(AnaMethod) %>%
  pivot_wider(id_cols = c(SparsMethod, n, epslevel.y, epslevel.g), names_from = AnaMethod, values_from = RMSPE.est) %>%
  rename("Noise(G)"="epslevel.g",
         "Thresholding"="SparsMethod") 
  

p = nested_loop_plot(resdf = tbl_loo1,
                     x = "n", steps = "epslevel.y",
                     grid_rows="Thresholding", grid_cols = "Noise(G)",
...
nfig = nfig+1
print(p)
ggsave(file = paste0(sim.path, "/fig_diagnostic_results.tiff"),
       width=1.75*150, height=1.25*150, units="mm", dpi=350, compression = "lzw")
```


<br>

```{r, eval=eval_diagnostic}
tbl_scens %>%
  filter(SparsMethod == "weight-based" & outcome =="diagnostic" & AnaMethod != "Oracle") %>%
  dplyr::select(setting, n, Adjust, AnaMethod,  epslevel.y, epslevel.g, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(setting, n, Adjust, AnaMethod,  epslevel.y, epslevel.g, RMSPE) %>%
  arrange(AnaMethod) %>%
  pivot_wider(names_from = AnaMethod, values_from = RMSPE) %>%
  relocate(setting, .before=n) %>%
  arrange(setting, Adjust, n, epslevel.y, epslevel.g) %>%
  kbl(caption="aRMSPE acrosss setings and data-generating mechanisms with 95% CI: weight-based sparsification)", escape = F) %>%
  row_spec(0, bold = T) %>%
  column_spec(1, bold=T) %>%
  column_spec(c(5), border_right = T) %>%
  row_spec(c(18), extra_css ="border-bottom: 1px solid") %>%
  kable_classic(full_width = T, html_font = "Calibri")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F, position = "left",fixed_thead = T)
```
...
```{r, eval=eval_diagnostic}
tbl_scens %>%
  filter(SparsMethod == "density-based" & outcome =="diagnostic" & AnaMethod != "Oracle") %>%
  dplyr::select(setting, n, Adjust, AnaMethod,  epslevel.y, epslevel.g, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(setting, n, Adjust, AnaMethod,  epslevel.y, epslevel.g, RMSPE) %>%
  ...
  relocate(setting, .before=n) %>%
  arrange(setting, Adjust, n, epslevel.y, epslevel.g) %>%
  kbl(caption="aRMSPE acrosss setings and data-generating mechanisms with 95% CI: density-based sparsification", escape = F) %>%
  row_spec(0, bold = T) %>%
  column_spec(1, bold=T) %>%
  column_spec(c(5), border_right = T) %>%
  row_spec(c(18), extra_css ="border-bottom: 1px solid") %>%
...
```{r, fig.align='center', fig.width=10, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
tbl_res$tbl_iters_res %>%
  filter(outcome=="diagnostic" & epslevel.y=="medium" & Adjust==T  & n==150 & AnaMethod != "Oracle") %>%
  mutate(AnaMethod = fct_relevel(AnaMethod, c("Oracle", "OPT", "AVG", "FLEX", "Null")),
         epslevel.g = paste0("Noise(G) = ", epslevel.g),
         epslevel.g = fct_relevel(epslevel.g, c("Noise(G) = none", "Noise(G) = medium", "Noise(G) = high"))) %>%
  ggplot(aes(x=AnaMethod, y=relRMSPE, fill=AnaMethod)) + geom_line(aes(group=AnaMethod), alpha=.05) +
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="", fig.fullwidth = TRUE, eval=eval_diagnostic}
cond <- quo((Adjust == TRUE & outcome=="diagnostic" & !AnaMethod %in% "Oracle"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(n, SparsMethod, epslevel.g, epslevel.y, AnaMethod, R2.est) %>%
  arrange(AnaMethod) %>%
  pivot_wider(id_cols = c(n, SparsMethod, epslevel.g, epslevel.y), names_from = AnaMethod, values_from = R2.est)

p = nested_loop_plot(resdf = tbl_loo1,
                     x = "n", steps = "epslevel.g",
                     grid_rows="SparsMethod", grid_cols = "epslevel.y",
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="", fig.fullwidth = TRUE, eval=eval_diagnostic}
cond <- quo((Adjust == FALSE & outcome=="diagnostic" & !AnaMethod %in% "Oracle"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.cap="Average functional form \\beta (t)\\ with the estimated flexible parametrization", eval=eval_diagnostic}
tbl_funcoeff %>%
  filter(outcome=="diagnostic"& Adjust==F & epslevel.g =="none" & n==150) %>%
  rename("Noise.Y"=epslevel.y) %>%
  ggplot(aes(x=fda.thresh, y=coeff.mean.std)) +
  geom_line() +
  scale_x_continuous("Threshold", breaks = seq(0,1,0.2)) +
  scale_y_continuous(expression("Average coefficient function "~hat(beta)(t))) +
  facet_grid(Noise.Y~SparsMethod, scales="free_y", labeller = label_both) +
  theme_bw() +
  theme(text = element_text(size=12),
                             axis.text.x = element_text(angle = 45,
                                                        vjust = 0.5,
                                                        size = 12))
ggsave(file = paste0(sim.path, "/fig_diag_FLEX_coefffun.tiff"),
       width=1.75*100, height=1*200, units="mm", dpi=350, compression = "lzw")
```

<br>


```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.cap="Average functional form \\beta (t)\\ with the estimated flexible parametrization adjusted by the second latent variable", eval=eval_diagnostic}
tbl_funcoeff %>%
  filter(outcome=="diagnostic"& Adjust==T & epslevel.g =="none" & n==150) %>%
  rename("Noise.Y"=epslevel.y) %>%
  ggplot(aes(x=fda.thresh, y=coeff.mean.std)) +
  geom_line() +
  scale_x_continuous("Threshold", breaks = seq(0,1,0.2)) +
