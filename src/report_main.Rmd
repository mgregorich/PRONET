...
cond <- quo((SparsMethod == "weight-based" & epslevel.g=="none") & 
...
tbl_loo1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(setting,n, dg.thresh, epslevel.y, AnaMethod, RMSE.est) %>%
  pivot_wider(id_cols = c(setting, n, dg.thresh, epslevel.y), names_from = AnaMethod, values_from = RMSE.est) %>% 
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & epslevel.y=="none") & 
                     ((adjust==F & setting %in% c("uni"))| (adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
cond <- quo((SparsMethod == "weight-based") & n==300 & epslevel.y=="none" &
                     ((adjust==F & setting %in% c("uni"))| (adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_gg1 <- tbl_scens %>%
  filter(!!cond) %>%
  dplyr::select(setting, dg.thresh, epslevel.g, AnaMethod, RMSE.est, RMSE.lo, RMSE.up) %>%
  rename("DGM"="dg.thresh")
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & n==300) & 
                     ((adjust==F & setting %in% c("uni"))| (adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "weight-based" & epslevel.g == "none" & epslevel.y == "none") %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "weight-based" & epslevel.g == "none" & epslevel.y == "high") %>%
  dplyr::select(dg.thresh, setting, adjust, AnaMethod, RMSE.est, RMSE.lo, RMSE.up) %>%
  rename("DGM"="dg.thresh") %>%
  mutate_at(vars(RMSE.est, RMSE.lo, RMSE.up), round_0, digits=2) %>%
  mutate(RMSE=paste0(RMSE.est, " (", RMSE.lo, " ,", RMSE.up, ")")) %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "weight-based" & epslevel.g == "high" & epslevel.y == "none") %>%
  dplyr::select(dg.thresh, setting, adjust, AnaMethod, RMSE.est, RMSE.lo, RMSE.up) %>%
  rename("DGM"="dg.thresh") %>%
  mutate_at(vars(RMSE.est, RMSE.lo, RMSE.up), round_0, digits=2) %>%
  mutate(RMSE=paste0(RMSE.est, " (", RMSE.lo, " ,", RMSE.up, ")")) %>%
...
cond <- quo(SparsMethod == "weight-based" & setting =="uni" & adjust==F & dg.thresh %in% c("single"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo5 <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & setting =="uni" & adjust==F & dg.thresh %in% c("single")) %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & setting =="uni" & adjust==F & dg.thresh %in% "random") %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, dg.thresh, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, epslevel.y, epslevel.g), names_from = dg.thresh, values_from = wmean) %>%
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "density-based" & setting =="uni" & adjust==F) %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, p, dg.thresh, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, p, epslevel.y, epslevel.g), names_from = dg.thresh, values_from = wmean) %>%
...
tbl_funcoeff %>%
  filter(SparsMethod == "weight-based" & setting =="uni" & adjust==F & epslevel.y =="none" & n==150) %>%
  rename("DGM"=dg.thresh) %>%
...
tbl_funs %>%
  filter(SparsMethod == "weight-based" & setting =="uni" & adjust==F & epslevel.g=="none" & epslevel.y =="none" & !(dg.thresh %in% c("single", "random"))) %>%
  rename("DGM"=dg.thresh) %>%
