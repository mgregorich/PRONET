...
tbl_scens <- readRDS(here::here(sim.path, "tbl_scenarios_results.rds")) %>%
  ...
  mutate(AnaMethod = fct_relevel(AnaMethod, c("Oracle", "Null", "bRMSE", "AVG", "FDA")))
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="Nested loop plot for RMSE with noise of Y = 0 and weight-based sparsification"}
tbl_loo1 <- tbl_scens %>%
...
print(p)
ggsave(file = paste0(sim.path, "/fig_loo_RMSE.tiff"),
       width=1.75*200, height=1*200, units="mm", dpi=350, compression = "lzw")
```

<br>

```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="Nested loop plot for RMSE with noise of G = 0 and weight-based sparsification"}
tbl_loo1 <- tbl_scens %>%
...
print(p)
ggsave(file = paste0(sim.path, "/fig_loo_R2.tiff"),
       width=1.75*200, height=1*200, units="mm", dpi=350, compression = "lzw")
```
...
In the following section, only the bRMSE analysis method is examined in more detail. Namely, the frequency of the selected threshold is investigated. For each data generation mechanism, the weighted mean threshold is calculated by weighting the mean value of the thresholds with the number of selections.

```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="Averaged threshold selection of the bRMSE approach with sequential weight-based sparsification under different data-generating mechanisms. The average threshold was calculated by a weighted mean with weights corresponding to the proportion of selection over the iterations."}
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based") %>%
...
p = nested_loop_plot(resdf = tbl_tfreq_loo,
                     ...
                     steps_y_base = -0.075, steps_y_height = 0.025,
                     x_name = "Sample size", y_name = "Average selected threshold",
                     spu_x_shift = 1.5,
                     legend_name = "DGmechanism",
                     ylim = c(0,0.6), y_breaks = seq(0,0.6,0.2),
...
print(p)
ggsave(file = paste0(sim.path, "/fig_loo_bRMSE_freq_wb.tiff"),
       width=1.75*200, height=1*200, units="mm", dpi=350, compression = "lzw")
```
<br>

```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="Threshold selection of the bRMSE approach with sequential density-based sparsification under different data-generating mechanisms. The average threshold was calculated by a weighted mean with weights corresponding to the proportion of selection over the iterations."}
tbl_tfreq_loo <- tbl_tfreq  %>%
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=6, fig.height=3, fig.cap="The three true functional forms used in the data-generating mechanism to generated the outcome based on a graph-theoretical feature sequence derived by sequential weight-based sparsification"}
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.cap="Averaged RMSE over 10 runs comparing the true functional form \\beta (t)\\ with the estimated flexible parametrization"}
tbl_funs %>%
  ...
  scale_y_continuous("RMSE averaged across 10 iterations") +
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.cap="Averaged RMSE over 10 runs comparing the true functional form \\beta (t)\\ with the standardized estimated flexible parametrization"}
tbl_funs %>%
  filter(eps.g==0 & eps.y ==0 & p ==100 & SparsMethod == "weight-based" & !(dg.thresh %in% c("single", "random"))) %>%
