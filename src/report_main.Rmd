...
cond <- quo((SparsMethod == "weight-based" & network.model=="block" & epslevel.g=="none" & Adjust==T))
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & network.model=="block" & epslevel.g=="none" & Adjust==F))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & network.model=="block" & epslevel.y=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
cond <- quo((SparsMethod == "weight-based") & network.model=="block" & n==300 & epslevel.y=="none" &
...
cond <- quo((SparsMethod == "weight-based" & network.model=="block" & n==300) & 
                     ((Adjust==F & setting %in% c("uni"))| (Adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & network.model=="block" & epslevel.g=="none") & 
                     ((Adjust==F & setting %in% c("uni"))| (Adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & network.model=="block" & epslevel.y=="none") & 
                     ((Adjust==F & setting %in% c("uni"))| (Adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
cond <- quo(SparsMethod == "weight-based" & network.model=="block" & setting =="uni" & Adjust==F & dg.thresh %in% c("single"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo5 <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & network.model=="block" & setting =="uni" & Adjust==F & dg.thresh %in% c("single")) %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & network.model=="block" & setting =="uni" & Adjust==F & dg.thresh %in% "random") %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, dg.thresh, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, epslevel.y, epslevel.g), names_from = dg.thresh, values_from = wmean) %>%
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & network.model=="block" & setting =="uni" & Adjust==F) %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, p, dg.thresh, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, p, epslevel.y, epslevel.g), names_from = dg.thresh, values_from = wmean) %>%
...
tbl_funcoeff %>%
  filter(SparsMethod == "weight-based" & network.model=="block" & setting =="uni" & Adjust==F & epslevel.y =="none" & n==150) %>%
  rename("DGM"=dg.thresh) %>%
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.cap="Average functional form \\beta (t)\\ with the estimated flexible parametrization"}
tbl_funcoeff %>%
  filter(SparsMethod == "weight-based" & network.model=="block" & setting =="uni" & dg.thresh %in% c("single", "random", "half-sine") & Adjust==F & epslevel.y =="none" & n==300) %>%
  rename("DGM"=dg.thresh) %>%
  rename("Noise.G"=epslevel.g) %>%
  ggplot(aes(x=fda.thresh, y=coeff.mean.ustd)) +
  geom_line() +
...
tbl_funs %>%
  filter(SparsMethod == "weight-based" & network.model=="block" & setting =="uni" & Adjust==F & epslevel.g=="none" & epslevel.y =="none" & !(dg.thresh %in% c("single", "random"))) %>%
  rename("DGM"=dg.thresh) %>%
