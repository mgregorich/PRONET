...
cond <- quo((setting=="uni" & outcome == "prognostic" & network.model=="block" & epslevel.g=="none" & Adjust==T))
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((setting=="uni" & outcome=="prognostic" & network.model=="block" & epslevel.y=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((setting=="uni" & outcome=="prognostic" & network.model=="block" & epslevel.y=="high" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
### Bivariable analyses with latent variable

```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((setting=="latent" & outcome=="prognostic" & network.model=="block" & epslevel.g=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((setting=="latent" & outcome=="prognostic" & network.model=="block" & epslevel.y=="none" & Adjust==T))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
```{r, fig.align='center', fig.width=14, fig.height=10, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((setting=="latent" & outcome=="prognostic" & network.model=="block" & epslevel.y=="none" & Adjust==F))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
cond <- quo((setting=="latent" & outcome=="diagnostic" & network.model=="block" & Adjust==T & DGM =="single"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
p = nested_loop_plot(resdf = tbl_loo1,
                     x = "n", steps = "epslevel.y",
                     grid_rows="SparsMethod", grid_cols = "epslevel.g",
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "density-based" & network.model=="block" & epslevel.g == "none" & epslevel.y == "none" & outcome =="prognostic") %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "density-based" & network.model=="block" & epslevel.g == "none" & epslevel.y == "high"& outcome =="prognostic") %>%
  dplyr::select(DGM, setting, Adjust, AnaMethod, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(DGM, setting, Adjust, AnaMethod, RMSPE) %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "density-based" & network.model=="block" & epslevel.g == "high" & epslevel.y == "none"& outcome =="prognostic") %>%
  dplyr::select(DGM, setting, Adjust, AnaMethod, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(DGM, setting, Adjust, AnaMethod, RMSPE) %>%
...
```{r}
tbl_scens %>%
  filter(n==300 & SparsMethod == "density-based" & network.model=="block" & epslevel.g == "high" & epslevel.y == "high" & outcome =="prognostic") %>%
  dplyr::select(DGM, setting, Adjust, AnaMethod, RMSPE.est, RMSPE.lo, RMSPE.up) %>%
  mutate_at(vars(RMSPE.est, RMSPE.lo, RMSPE.up), round_0, digits=2) %>%
  mutate(RMSPE=paste0(RMSPE.est, " (", RMSPE.lo, " ,", RMSPE.up, ")")) %>%
  select(DGM, setting, Adjust, AnaMethod, RMSPE) %>%
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=12, fig.height=8, fig.cap="", fig.fullwidth = TRUE}
cond <- quo((SparsMethod == "weight-based" & outcome=="diagnostic" & network.model=="block" & DGM=="single") & 
                     ((Adjust==F & setting %in% c("uni"))| (Adjust==T & setting %in% c("latent", "multi"))))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
cond <- quo(SparsMethod == "weight-based" & outcome=="prognostic" & network.model=="block" & setting =="uni" & Adjust==F & DGM %in% c("single"))
cond_sub <- str_remove_all(as.character(cond)[2], "[[:punct:]]")
cond_sub <- str_replace_all(cond_sub, "  ", ", ")
cond_sub <- str_replace_all(cond_sub, "==", "=")
...
tbl_loo5 <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic" & network.model=="block" & setting =="uni" & Adjust==F & DGM %in% c("single")) %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
...
tbl_tfreq_loo <- tbl_tfreq  %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic" & network.model=="block" & setting =="uni" & Adjust==F & DGM %in% "random") %>%
  mutate_at(vars(Thresh, count), as.numeric) %>%
  group_by(n, DGM, epslevel.g, epslevel.y) %>%
  summarise(wmean = weighted.mean(Thresh, count)) %>%
  pivot_wider(id_cols = c(n, epslevel.y, epslevel.g), names_from = DGM, values_from = wmean) %>%
...
```{r, echo=F, warning=F, message=F, fig.align='center', fig.width=8, fig.height=6, fig.cap="Average functional form \\beta (t)\\ with the estimated flexible parametrization"}
tbl_funcoeff %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic" & network.model=="block" & setting =="uni" & DGM %in% c("single", "random", "half-sine") & Adjust==F & epslevel.y =="none" & n==300) %>%
  rename("Noise.G"=epslevel.g) %>%
  ggplot(aes(x=fda.thresh, y=coeff.mean.ustd)) +
  geom_line() +
  scale_x_continuous("Threshold", breaks = seq(0,1,0.2)) +
...
tbl_funs %>%
  filter(SparsMethod == "weight-based" & outcome=="prognostic" & network.model=="block" & setting =="uni" & Adjust==F & epslevel.g=="none" & epslevel.y =="none" & !(DGM %in% c("single", "random"))) %>%
