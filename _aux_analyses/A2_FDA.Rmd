...
df <- generate_data(n=sparams$n, p=sparams$p, q=sparams$q, mu=sparams$mu, alpha=sparams$alpha, delta=sparams$delta, 
                    ...
                    beta0=sparams$beta0,xbeta=sparams$xbeta, gbeta = sparams$gbeta, eps=sparams$eps, sthresh=sparams$sthresh)
...
list.gvars <- lapply(1:nrow(data.network), function(x) 
  data.frame("Subj"=x, wrapperThresholding(eweights=data.network[x,], msize=p, tseq=da_thresh)))
...
data.bRMSE %>%
  ...
  theme_bw() +
  facet_grid(SparsMethod~ThreshMethod) +
  geom_vline(data = data.frame(xint=dg_thresh,SparsMethod="weight-based", ThreshMethod="trim"), 
