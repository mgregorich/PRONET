...
```{r, echo=T}
fit.fda <- refund::pfr(Y ~ lf(X, k = 5, bs="bs"), method="REML", data=data.FDA, family="gaussian")
fit.fda
```
...
coef <- coef(fit.fda)
plot(fit.fda, main="Estimated coefficient function b(x)", xlab="Threshold", ylab=expression(paste(beta(t))), lwd=1.2)
