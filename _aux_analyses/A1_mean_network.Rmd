...
df.sparams  %>%
  kbl(caption="Parameters for the data-generating mechansm", escape = F) %>%
...
mnet = abs(mnet)
data.mnet <- data.frame(t(sapply(thresh.seq, function(x){
  mat <- weightThresholding(adj=mnet, w = x)$adj
  cc <- c("Thresh"=x,calcGraphFeatures(adj=mat))
  return(cc)
  })))
...
df <- evalIndivNetworkVariability(n=sparams$n, p=sparams$p, q=sparams$q, alpha=sparams$alpha, 
                                  obeta0=sparams$obeta0, delta,beta0, tseq=thresh.seq)

data.indivnet <- df$FMI
data.indivnet$Type <- "indiv"
data.mnet.melted <- melt(data.mnet, id.vars = "Thresh")
data.mnet.melted$Subj <- 0
data.mnet.melted$Type <- "mean"

df <- data.frame(rbind(data.mnet.melted, data.indivnet)) %>%
...
for(i in 1:length(gvars)){
 plot.list[[i]] <- df %>%
    ...
    mutate_at(c(2,4,5), as.factor) %>%
    filter(variable %in% gvars[i]) %>%
