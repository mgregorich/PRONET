...
beta.shape = list("shape1"=5, "shape2"=2) # shape parameters of the beta distribution
alpha.norm <- list("mean"=1, "std"=0.5)
alpha.unif <- list("min"=0, "max"=0.2)
...
calc_eta_mean_and_var <- function(norm.pars, unif.pars){
  ...
  V=norm.pars$std^2 + 2*((unif.var +(unif.mean^2)*1)-(unif.mean^2*0))
...
gen_quantiles <- function(alpha0, alpha1, alpha2, X1, X2, beta.pars, eta.pars){
  ...
  p = pnorm(eta, mean=eta.pars$mean, sd=sqrt(eta.pars$var))
  q = qbeta(p, beta.pars$shape1, beta.pars$shape2)
  return(data.frame("eta"=eta, "p"=p, "q"=q))
}


alpha0 = rnorm(p0, alpha.norm$mean, alpha.norm$std)
alpha1 = runif(p0, alpha.unif$min, alpha.unif$max)
alpha2 = runif(p0, alpha.unif$min, alpha.unif$max)

X1=rnorm(n,0,1)
X2=rnorm(n,0,1)

eta.distr <- calc_eta_mean_and_var(norm.par = alpha.norm, unif.par = alpha.unif)
vec <-lapply(1:n, function(x) data.frame("Subj"=x, gen_quantiles(alpha0, alpha1, alpha2, X1[x], X2[x], 
                                                                 beta.pars = beta.shape, eta.pars = eta.distr)))
