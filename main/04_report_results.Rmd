...
df.sparams <- data.frame("Terms"=c("Sample size", "Nodes",  "Possible edges","Latent processes",
                                   "Barabasi-Albert graph density", "True threshold $\\tau$", "True $R^2$","$\\sigma_y$: noise added to Y","$\\sigma_g$ - noise added to each graph"),
                         "Values"=c(n,p,po,q, d, dg.thresh, round(list_results$true.R2,2), eps.y, eps.g))
df.sparams  %>%
